...
async def aria_start():
    ...
    torlog.debug(stdout)
    torlog.debug(stderr)
    return aria2p.API(aria2p.Client(host="http://localhost", port=8100, secret=""))
...
def add_url(aria_instance, text_url, c_file_name):
    ...
    # Add URL Into Queue
    try:
        download = aria_instance.add_uris(uris)
    except Exception as e:
        return (
            False,
            "**FAILED** \n" + str(e) + " \nPlease do not send SLOW links. Read /help",
...
async def check_progress_for_dl(
    ...
    try:
        ...
        if complete := file.is_complete:
            await event.edit(
                f"Download completed: <code>{file.name}</code> to path <code>{file.name}</code>",
                parse_mode="html",
                buttons=None,
            )
