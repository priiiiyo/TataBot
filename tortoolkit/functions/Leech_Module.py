...
from . import QBittorrentWrap, ariatools
    ...
    if msg is None:
        ...
        if msg.raw_text.lower().startswith("magnet:"):
            ...
            # Zippyshare
                ...
                name = match.group(3)
                url = "https://www{}.zippyshare.com/d/{}/{}/{}".format(
                    server, id_, val, name
                )
            ...
            # yadisk
                ...
                try:
                    link = re.findall(
                        r"\b(https?://.*(yadi|disk)\.(sk|yandex)*(|com)\S+)", urls
                    )[0][0]
                    print(link)
                except:
...
async def handle_ext_zip(path, rmess, omess):
    ...
    start = time.time()
    await rmess.edit(
        f"{rmess.text} Trying to Extract the archive with password <code>{password}</code>.",
        parse_mode="html",
    )
    wrong_pwd = False
    ...
    while True:
        if not wrong_pwd:
            ...
            if (time.time() - start) > 1200:
                await rmess.edit(
                    f"{rmess.text} Extract failed as no correct password was provided uploading as it is."
                )
                return False
